...
bool Triangle::isPointIn(int x, int y) const
{
	...
	Triangle t2(getPointAtIndex(2).x, getPointAtIndex(2).y, getPointAtIndex(0).x, getPointAtIndex(0).y, p.x, p.y);
	Triangle t3(getPointAtIndex(2).x, getPointAtIndex(2).y, getPointAtIndex(1).x, getPointAtIndex(1).y, p.x, p.y);

	return abs(t1.getArea() + t2.getArea() + t3.getArea()) <= std::numeric_limits<double>::epsilon();
}